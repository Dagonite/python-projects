...
'''
9.
This exercise and the following one help to prepare you for the Python assignment.
The table below shows 10 different “patch designs”. Each patch design features a
regular arrangement of lines, circles, rectangles and/or text, and has dimensions
of 100×100 pixels.

Write a function draw_patch_window (without parameters) which displays, in a graphics
window of size 100×100 pixels, the patch design which is labelled with the final
digit of your student number. The patch design should be displayed in red, as in
the table. It’s important that your program draws the patch design accurately, but
don’t worry if one pixel is chopped off at the edge of the window. Note that you
don’t need to draw a border around the patch design.
'''
...
'''
10.
Write a function draw_patch which draws the same patch design, but which takes four
parameters: the window in which to draw the patch, the x and y coordinates of where
the top-left corner of the patch should be, and the colour of the patch.
'''
...
'''
Write another function draw_patchwork (without parameters) which uses draw_patch
to draw a blue “patchwork” three patches wide and two patches high in a graphics
window of size 300 × 200 pixels.
'''
def draw_patchwork():
...
'''
11.
Write an eyes_all_around function that allows the user to plot exactly 30 eyes on
a graphics window of dimensions 500 by 500 by clicking on chosen centre points.
All eyes should be of radius 30, but they should be of different colours:
specifically, the colours should repeatedly cycle through the sequence “blue”,
“grey”, “green” and “brown”. Your function should call draw_coloured_eye to draw
each eye.
'''
def eyes_all_around():
...
'''
12. [harder]
Write an archery_game function. This function should draw a target (like that from
pract03) using the supplied draw_circle function. Your function should then allow
the user to click on the graphics window five times, representing the firing of
five arrows – each click representing the point on the target that is aimed at.
...
Each time the user clicks, the function should (i) display a small black circle
representing where the arrow hits, and (ii) record the number of points scored.
The points awarded for each arrow (click) are as follows: 10 for the yellow area,
5 for red and 2 for blue. After the final arrow, the function should display the
total score on the graphics window. (Hint: calculate distances by first importing
your pract05.py file and using the distance_between_points function.
'''
def archery_game():
