...
from graphics import GraphWin, Line, Point, Circle, Rectangle, Text, color_rgb
...
Assume that a valid name is any string of alphabetic characters only, such as
“Jenny”. If the user enters an invalid name, the function should continue to
ask the user for a name until she/he enters a valid one. Once a valid name
has been entered, this should be returned. (Hint: use the isalpha method
discussed above.) '''
...
continuously cycle through the standard red → red/amber → green → amber → red
sequence, simulating a real set of traffic lights. Use “yellow” for amber,
and add realistic delays between light changes using the sleep function from
the time module. E.g.,
...
for a pupil’s coursework based on a mark. Write a function grade_coursework
that asks the user for a mark and, using a call to calculate_grade, displays
a “The pupil achieved a grade of ...” message including the grade achieved.
The function should only display a grade for valid marks (i.e. between 0 and
20) – if the user enters an invalid mark, they should be re-prompted until
...
goods. The function should repeatedly ask the user for (i) the unit price of
a product in the order, (ii) the quantity of that product in the order,
and (iii) whether there are any more products in the order. When the user has
completed entering prices & quantities, the function should output a message
containing the total order price to 2 decimal places. '''
...
def order_price():
    ...
    quantity = 0
    while True:
        unit_price = eval(input("Enter the unit price of a product (0 to "
                                "stop): "))

        while unit_price > 0:
            quantity = round(eval(input("Enter the quantity of the product (0 "
                                        "to stop): ")))
...
note: I've set my own coords so this will be different) within a graphics
window of sufficient size. The function should then respond to each user
click on the eye by displaying (underneath the eye) the name of the part of
the eye clicked on (i.e. one of “pupil”, “iris”, “sclera” (or “white”)). The
user should be able to finish (i.e. close the window) by clicking on any
point outside the eye. (Hint: use your draw_brown_eye and
...
celsius_to_fahrenheit for converting between temperature units. Using calls
to these two functions, write a temperature_converter function that provides
a text-based interface which allows the user to (repeatedly) convert
temperature values until he/she wishes to stop. The user should be asked
which way the conversion should be performed separately for each conversion. '''
...
def temperature_converter():
    while True:
        ...
        if conv_type == "ftc" or conv_type == "ctf":
            while True:
                temperature = input("Enter the temperature in degrees (whole "
                                    "number): ")
...
then allow the user to guess the number. After each incorrect guess,
it should display “Too high” or “Too low” as appropriate. If the user guesses
...
of the points of two players in a game of table tennis. In table tennis,
the first player to reach 11 points wins the game; however, a game must be
won by at least a two point margin. The points for the players should be
displayed on two halves of a graphics window, and the user clicks anywhere on
the appropriate side to increment a player’s score. As soon as one player has
won, a “wins” message should appear on that side. '''
...
def table_tennis_scorer():
    ...
    while not((left_score >= 11 or right_score >= 11) and
              (left_score >= (right_score + 2) or
               right_score >= (left_score + 2))):
        cursor = win.getMouse()
        if cursor.getX() <= .5:
            left_score += 1
...
parameters columns and rows, and displays a rows × columns grid of blue eyes
(all of radius 50) within a box (rectangle). There should be a border of size
50 between the box and the edge of the window. For each click of the mouse
inside the box, the function should behave as follows: if the click is on an
eye, the row and column of that eye should be displayed in the space below
the box, for example as shown below given the click denoted by the dot (note
that row and column numbers should begin at 1):
...
the first) the user misses the circle, a “getting closer” or “getting further
away” message should be displayed (depending on the position of the current
and previous clicks). If the user manages to find the circle (by clicking
within its circumference), then the circle should be displayed and the user
given some points: 10 points for finding it with the first click, down to 1
point for finding it with the 10th click. The game then restarts. However,
each time the game restarts the circle should be given a new random position
and its radius reduced by 10%. The game ends when the user fails to find the
circle within 10 clicks. The total number of points scored should then be
displayed (note: game is too hard with radius 30 circle so I've uped it to
60). '''
