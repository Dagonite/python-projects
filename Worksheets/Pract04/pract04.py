...
'''
1.
Write a personal_greeting function which, after asking for the user’s name,
...
'''
2.
Write a formal_name function which asks the user to input his/her given name
and family name, and then outputs a more formal version of their name. E.g. on
input Sam and Brown, the function should output S. Brown (again note the spacing
and punctuation).
'''
def formal_name():
    first_name = input("Enter your first name: ")
...
'''
3.
Copy the kilos_to_pounds conversion function from your pract01.py file. Modify
this function so that its output takes the form of a message such as “12.34 kilos
is equal to 27.15 pounds”, where both the user’s kilos value and the calculated
pounds values are displayed to two decimal places.
'''
def kilos_to_pounds():
...
'''
4.
Suppose the University decides that students’ email addresses should be made up
of the first 4 letters of their surname, the first letter of their forename, and
the final two digits of the year they entered the university, separated by dots.
...
def generate_email():
    ...
    email = (last_name[0:4] + "." + first_name[0] + "." + entry_year[2:] + "@myport.ac.uk").lower()
    print(email)
...
'''
6.
Write a function graphic_letters which first asks the user to enter a word, opens
a graphics window, and then allows the user to display the letters of the word at
different locations by clicking the mouse once for each letter. (Use the setSize
method of the Text class to make the letters appear big.)
'''
def graphic_letters():
...
'''
7.
Write a sing_a_song function which outputs a “song” based on a single word. The
user should be asked for the song’s word, how many lines long the song should be,
and how many times the word should be repeated on each line. For example, if the
user enters the word “dum” and the numbers 2 and 4, the function should then output
the following song (note that the spaces are important):
    dum dum dum dum
    dum dum dum dum
'''
def sing_a_song():
    ...
    repeats = eval(input("Enter how many times the word is repeated on each line: "))
...
'''
8.
Write a function exchange_table that gives a table of euros values and their
equivalent values in pounds, using an exchange rate of 1.108 euros to the pound.
The euros values should be 0, 1, 2, . . . , 20, and should be right justified.
The pounds values should be 6 right justified and given to two decimal places
(i.e. with decimal points lined up and with pence values after the points).
'''
def exchange_table():
...
'''
9.
Write a make_acronym function that allows the user to enter a phrase, and then
displays an acronym (the first letters of the words in capitals) for that phrase.
For example, if the user enters “University of Portsmouth”, the function should
display UOP. (Hint: first use the split method to find the words in the inputted
string.)
'''
def make_acronym():
...
'''
10. [harder]
Write a name_to_number function that asks the user for their name and converts it
into a numerical value by adding up the “values” of its letters (where ‘a’ is 1,
‘b’ is 2, . . . ‘z’ is 26). So, for example, “Sam” has the value 19 + 1 + 13 = 33.
'''
def name_to_number():
...
'''
11.
Write a file_in_caps function which displays the contents of a file in capital
letters. The name of the input file should be entered by the user.
'''
def file_in_caps():
    out_file = open("Battles - Atlas.txt", "w")
    print("People won't be people\nWhen they hear this sound\nThat's been glowing in the dark at the edge of town", file=out_file)
    out_file.close()
...
'''
Now write a graphical version graphical_rainfall_chart that displays a similar
bar chart in a graphical window but uses filled rectangles instead of sequences
of asterisks.
'''
