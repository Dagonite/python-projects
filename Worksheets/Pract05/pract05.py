...
parameter representing a circle’s radius, and returns the area of the circle. 
Write a similar function called circumference_of_circle that has a radius 
...
circumference of the circle (displayed to three decimal places); e.g. if the 
user enters a radius of 5, then the output message might be: 

    The area is 78.540 and the circumference is 31.416
     
Your function should call both the area_of_circle and the 
circumference_of_circle functions to do the calculations. '''
def circle_info():
    radius = eval(input("Enter the radius of the circle: "))
    area = area_of_circle(radius)
    circumference = circumference_of_circle(radius)
    print("The area is {0:0.3f} and the circumference "
          "is {1:0.3f}".format(area, circumference))
...
draw_brown_eye_in_centre function so that it calls draw_circle three times in 
order to draw a brown “eye” in the centre of a graphics window. The radii of 
the white, brown and black circles should be 60, 30 and 15 respectively '''
...
For example, the function call draw_block_of_stars(5, 3) should result in the 
following output: 
...
'''Now, write a function drawLetterE that displays a large capital letter E; 
for example: 
...
your function. Now, by using your completed draw_brown_eye function, write a 
draw_pair_of_brown_eyes function (without parameters) that draws a pair of 
eyes on a graphics window. '''
...
'''6. Write a function distance_between_points that has two parameters p1 and 
p2, each of type Point, and returns the distance between them. This function 
should use the formula for Pythagoras’ Theorem, as in practical worksheet 
P02. For example, the function call: 

    distance_between_points(Point(1, 2), Point(4, 6))

should result in the value 5.0 being returned. (Hint: you’ll need to use the 
getX and getY methods to get the x and y coordinates of points p1 and p2.) '''
...
then spaces and finally asterisks, all of the same height). The widths of the 
four rectangles and their common height should be parameters. E.g., a call: 
draw_blocks(0, 5, 4, 3, 2) will result in the output: 

    *****    ***
    *****    ***

(with no space before the first asterisks due to the 0 argument). Now, 
write a function draw_letter_A that uses draw_blocks in order to display a 
large capital A in asterisks, such as: 
...
on the outer circumference of this eye. (Hint: This function should call the 
distance_between_points function from exercise 6 to obtain the radius of each 
eye, as well as the draw_brown_eye function from exercise 5 to draw the 
eyes.) '''
...
with spaces between each character (for example, hello would be displayed as 
...
function should first open a graphics window. It should then ask the user for 
six strings, displaying them on the graphics window as they are entered. The 
strings should be displayed in upper case, and from the top of the window to 
the bottom with descending point sizes of 30, 25, 20, 15, 10 and 5. (Make 
sure that the lines are well-spaced out — you might need to experiment a 
little with spacing.) '''
...
positions. Begin by copying your draw_stick_figure function from pract03.py, 
and changing it so that it has three parameters, representing a graphics 
window, the position of the figure (a Point) and its size (an int). (What the 
position and size mean exactly is up to you.) Your draw_stick_figure_family 
function should contain just four or five calls to the modified version of 
draw_stick_figure. '''
